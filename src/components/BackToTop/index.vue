...
export default {
  ...
  mounted () {
    if (this.element === 'window') {
      ...
      this.targetElement = document.querySelector(this.element)
  ...
  methods: {
    handleScroll () {
      this.targetElement === window ? this.visible = this.targetElement.pageYOffset > this.visibilityHeight : this.visible = this.targetElement.scrollTop > this.visibilityHeight
    },
    backToTop () {
      if (this.isTop) return
      this.isTop = true
      const start = this.targetElement.pageYOffset || this.targetElement.scrollTop
      let i = 0
      this.interval = setInterval(() => {
        const next = Math.floor(this.easeInOutQuad(10 * i, start, -start, 300))
        if (next <= this.backPosition) {
          this.targetElement.scrollTo(0, this.backPosition)
          clearInterval(this.interval)
          setTimeout(() => {
            this.isTop = false
          }, 500)
